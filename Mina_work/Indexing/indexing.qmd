...
Because `WVAR` public school suspensions and `YLCV` public school enrollment are in counts, I decided to omit them since areas with larger populations will be overly represented/punished, meaning they would add extra convolution to the index. If we can get those numbers in percentages, that would be a different story.
