...
```{r}
# Here I (ChatGPT) writes a function to do the indexing for me so I don't have to hard code the data frames every single time sorry guys i cant code... TT
...
names(balls)[5:9] <- c("EDA", "EDB", "EDC", "EDE", "EDG") # get rid of the stupid date things since they're all the same (see below)
...
This layer includes the last two variables detailed in the legend section, which have been excluded due to them being in counts.
...
names(CnT) <- colnames
names(CnT)[5:11] <- c("EDA", "EDB", "EDC", "EDE", "EDG", "WVAR", "YLCV") # get rid of the stupid date things since they're all the same (see below)
...
```{r}
# Get index using function we wrote above
CnT_index <- interpolate_scores(condition, CnT, vars_cond)|> arrange(desc(Index))
```

```{r}
# Top 10 indexes
kable(head(CnT_index, 10), caption = "Top 10 indexes (by City or Town)", digits=3)
```

```{r}
# Bottom 10 indexes
kable(tail(CnT_index, 10), caption = "Bottom 10 indexes (by City or Town)", digits=3)
```

# Indexing - Zip Codes

This layer also includes the last two variables detailed in the legend section, which have been excluded due to them being in counts.
...
names(zc) <- colnames
names(zc)[7:13] <- c("EDA", "EDB", "EDC", "EDE", "EDG", "WVAR", "YLCV") # get rid of the stupid date things since they're all the same (see below)
...
```{r}
# Get index using function we wrote above
zc_index <- interpolate_scores(condition, zc, vars_cond)|> arrange(desc(Index))
```

```{r}
# Top 10 indexes
kable(head(zc_index, 10), caption = "Top 10 indexes (by Zip Code)", digits=3)
```

```{r}
# Bottom 10 indexes
kable(tail(zc_index, 10), caption = "Bottom 10 indexes (by Zip Code)", digits=3)
```

# Indexing - Counties

This layer includes all the variables detailed in the legend section, but for simplicity's sake I will only be looking at the `EDX` variables as mentioned previously.
...
names(cntz) <- colnames
names(cntz)[5:13] <- c("CHF", "CHS", "EDA", "EDB", "EDC", "EDE", "EDG", "WVAR", "YLCV") # get rid of the stupid date things since they're all the same (see below)
...
```{r}
# Get index using function we wrote above
cntz_index <- interpolate_scores(condition, cntz, vars_cond)|> arrange(desc(Index))
cntz_index |> kable(caption = "Indexes (by County)")
```

# Indexing - Regions

This layer also includes all the variables detailed in the legend section, but for simplicity's sake I will only be looking at the `EDX` variables as mentioned previously.
...
names(rgns) <- colnames
names(rgns)[5:13] <- c("CHF", "CHS", "EDA", "EDB", "EDC", "EDE", "EDG", "WVAR", "YLCV") # get rid of the stupid date things since they're all the same (see below)
...
```{r}
# Get index using function we wrote above
rgns_index<- interpolate_scores(condition, rgns, vars_cond)|> arrange(desc(Index))
rgns_index |> kable(caption = "Indexes (by Region)")
```

```{r}
# change GEOID and stuff to be a chr string so they can be put into bind_rows
balls_index <- balls_index |> mutate(GEOID = as.character(GEOID))
CnT_index <- CnT_index |> mutate(GEOID = as.character(GEOID))
cntz_index <- cntz_index |> mutate(GEOID = as.character(GEOID))
zc_index <- zc_index |> mutate(GEOID = as.character(GEOID)) |> mutate(Name = as.character(Name))
```

```{r}
# bind all the indexes together and export as csv
index_final <- bind_rows(balls_index, CnT_index, cntz_index, zc_index, rgns_index)
write.csv(index_final, file = "D:/RStudio/Projects/FVDE/Mina_work/Indexing/lifelong_learning_indexes.csv", row.names = FALSE)
```
