...
```{r}
# Top 10 indexes
kable(head(CnT_index, 10), caption = "Top 10 indexes (by Census Tract)", digits=3)
```

```{r}
# Bottom 10 indexes
kable(tail(CnT_index, 10), caption = "Bottom 10 indexes (by Census Tract)", digits=3)
```
...
```{r}
# Top 10 indexes
kable(head(zc_index, 10), caption = "Top 10 indexes (by Census Tract)", digits=3)
```

```{r}
# Bottom 10 indexes
kable(tail(zc_index, 10), caption = "Bottom 10 indexes (by Census Tract)", digits=3)
```
...
```{r}
# Get index using function we wrote above
interpolate_scores(condition, cntz, vars_cond)|> arrange(desc(Index)) |> kable()
```
...
```{r}
# Get index using function we wrote above
interpolate_scores(condition, rgns, vars_cond)|> arrange(desc(Index)) |> kable()
```
